...
const checkActiveClassPlayer = (diceNumber) => {
	if (main_wrapper.children[activePlayer].classList.contains("active")) {
		...
		if (activePlayer) {
			...
			console.log(
				`current value is ${currentValue_1} and dice number is ${diceNumber}`
			);
		...
		//Returns the current value for player-1
			...
			console.log(
				`current value is ${currentValue_0} and dice number is ${diceNumber}`
			);
...
rollDiceBtn.addEventListener("click", () => {
	...
	if (diceNumber > 1) {
		...
		main_wrapper.children[
			activePlayer
		].children[2].children[1].textContent = checkActiveClassPlayer(diceNumber);
	} else {
		diceImage.src = diceImageObject[diceNumber];
...
holdDiceBtn.addEventListener("click", () => {
	...
	if (activePlayer == 0) {
		globalValue_0 += parseInt(
			main_wrapper.children[activePlayer].children[2].children[1].textContent
		);
...
setInterval(function () {
	if (globalValue_0 >= 100) {
		...
		main_wrapper.children[0].children[0].style.cssText =
			" font-weight: bolder;transform: scale(1.5);color:#0f9b0f;";
		document.cookie = globalValue_0;
	} else if (globalValue_1 >= 100) {
		playerWinsAudio.play();
		disableRollAndHoldBtn();
		main_wrapper.children[1].children[0].textContent = "Winner";
		main_wrapper.children[1].children[0].style.cssText =
			" font-weight: bolder;transform: scale(1.5);color:#0f9b0f;";
